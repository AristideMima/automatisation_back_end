...
#     # Get task and async result
#     task_id = request.data['task_id']
#     res = AsyncResult("your-task-id")
#
#     if res.ready():
#         data_response = res.get()
#         return Response({'result': data_response})
#
...
#     result = make_calcul_async.delay(user_id, request_obj)
#
#     return Response({'result_id': result.id}, status=200)
#
#
# @shared_task(bind=True)
# def make_calcul_async(self, user_id, request):
#     # Get data passed by request
#     # We will compute all data separately
#
#     # Load parameters
#     try:
#         accounts = pd.DataFrame(request['accounts'])
#         options = request['options']
#         operations = pd.DataFrame(request['operations'])
...
#         commision_mouvement = request['com_mvt']
#         commission_decouvert = request['com_dec']
#         date_deb = pd.to_datetime(options["date_deb"])
#         date_fin = pd.to_datetime(options["date_fin"])
#         type_arrete = request['type_arrete']
#         ordre = request['ordre']
#         int_epargne = request['int_epargne']
#     except Exception as e:
#         print(e)
#         return {"message": "Paramètres requis absents"}
#
#     print()
#     # Load latest history
#     try:
#
#         try:
#             req = ActiveFileUser.objects.get(user_id=user, type="historique")
...
#             except Exception as e:
#                 print(e)
#                 return {"message": "Aucun Fichier historique présent"}
...
#         print(e)
#         return {"message": "Erreur chargement fichier Historique"}
#
#     # Load Journal
#     try:
#
#         try:
#             req = ActiveFileUser.objects.get(user_id=user, type="journal")
...
#     # Check result len
#     if len(history) == 0:
#         return {"message": "Aucun historique sur cette date pour les comptes choisis"}
#
#     if ordre:
...
#         df = df.merge(pd.DataFrame(operations)[['code_operation', 'com_dec', 'com_mvt']], on="code_operation",
#                       how="left")
#
