...
def make_calcul_inter(self, user_id, request):
    ...
    if type_account == "Courant":
        for index, account in tqdm(accounts[:500].iterrows(), total=accounts[:500].shape[0]):
            ...
            try:
                ...
                journal_copie = journal_copie["Numero de compte"].apply(lambda x: x.split("-")[0])
...
def make_calcul(request):
    ...
    result = make_calcul_inter.delay(user_id, json_request)
