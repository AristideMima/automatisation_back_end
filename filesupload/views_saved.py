...
def make_calcul(request):
    ...
    # history = history[((last_month <= history['Date Comptable']) & (history['Date Comptable'] <= next_mont))]

    history = history[((date_deb <= history['Date de Valeur']) & (history['Date de Valeur'] <= date_fin)) & (
            (date_deb <= history['Date Comptable']) & (history['Date Comptable'] <= date_fin)
    )]
        ...
        for index, account in tqdm(accounts[:500].iterrows(), total=accounts[:500].shape[0]):
            ...
            try:
                ...
                account_journal_value = \
                    journal_copie[journal_copie["Numero de compte"] == account["num_compte"].split("-")[1]]['Net client'].iloc[0]
                ecart = (second['TOTAL'][-1]) - account_journal_value
                data_compressed = {"id": index, "N° Compte": account["num_compte"], "Calcul": second['TOTAL'][-1], "Journal": account_journal_value, "Ecart": ecart, "date_deb": options["date_deb"],
...
def get_infos(request):
    ...
    try:
        try:
            req = ActiveFileUser.objects.get(user=user.id, type="historique")
            last_history = File.objects.get(id=req.file.id)
        except Exception as e:
            print(e)
            try:
